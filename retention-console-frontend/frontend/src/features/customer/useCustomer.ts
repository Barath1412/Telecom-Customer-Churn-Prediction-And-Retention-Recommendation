...
export function useCustomer(id: string) {
  return useQuery({ queryKey: qk.customer(id), queryFn: () => api.customer(id), enabled: !!id })
}

/**
 * No optimistic update here, deliberately. Approving an offer is a written
 * audit record; showing it as done before the server confirms would let an
 * agent believe an action was logged when it was not.
 */
