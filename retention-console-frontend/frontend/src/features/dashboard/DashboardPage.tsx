  ...
  return (
    <div className="space-y-4">
      ...
      <Card title="Decision funnel" subtitle="Every customer scored in this run">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={funnel} margin={{ top: 4, right: 8, bottom: 4, left: 8 }}>
              <CartesianGrid
                ...
                vertical={false}
              />
              <XAxis dataKey="stage" tick={{ fontSize: 12 }} stroke="var(--color-ink-3)" />
              <YAxis tick={{ fontSize: 12 }} stroke="var(--color-ink-3)" width={48} />
              <Tooltip
                contentStyle={{
                  fontSize: 12,
                  borderRadius: 7,
        ...
        {/* Charts are decorative for assistive tech; the table is the accessible path. */}
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <caption className="sr-only">Allocation parity across demographic groups</caption>
            <thead>
              <tr className="border-b border-line-strong text-left text-ink-2">
            ...
            <tbody>
              {data.allocation_parity.map((r) => (
                <tr key={`${r.attribute}-${r.group}`} className="border-b border-line last:border-0">
                  ...
                  <td>{r.group}</td>
                  <td className="num text-right">{r.n.toLocaleString('en-US')}</td>
                  <td className="num text-right">{r.queued.toLocaleString('en-US')}</td>
